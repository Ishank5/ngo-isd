...
function displayNoCampState() {
    document.getElementById('campCard').innerHTML = `
        <div class="no-camp-state">
            ...
            <p>Please select a camp to start dispensing medicines</p>
...
async function loadReadyPatients() {
    ...
    try {
        ...
        patients.sort((a, b) => {
            const aTime = a.journeyStatus?.doctor?.timestamp || '';
            const bTime = b.journeyStatus?.doctor?.timestamp || '';
    ...
    if (patients.length === 0) {
        readyPatientsList.innerHTML = `
            <div class="empty-state">
                ...
                <p>No patients are currently ready for medicine dispensing</p>
            </div>
        `;
        return;
    }
...
async function loadPatientPrescription(patient) {
    try {
        ...
        const visitData = visitDoc.data();
        const consultationData = visitData.journeyStatus?.doctor?.data;
        
        if (!consultationData || !consultationData.medicines) {
            document.getElementById('prescriptionList').innerHTML = `
                <div class="empty-state">
                    <div class="empty-state-icon">📋</div>
                    <h3>No Prescription Found</h3>
                    <p>No medicines were prescribed for this patient</p>
                </div>
            `;
            return;
        }
        
        displayPrescription(consultationData.medicines);
        generateDispensingForm(consultationData.medicines);
...
function generateDispensingForm(medicines) {
        ...
        dispensingItem.innerHTML = `
            <div class="dispensing-header">
                ...
                <div class="dispensing-toggle">
                    <input type="checkbox" id="dispense-${index}" class="dispensing-checkbox" checked onchange="toggleDispensingFields(${index})">
            ...
            <div class="dispensing-details" id="dispensing-details-${index}">
                <div class="dispensing-field">
                    ...
                    <input type="number" name="quantity-${index}" min="1" required>
                </div>
                <div class="dispensing-field">
                    <label>Unit</label>
                ...
                <div class="dispensing-field">
                    ...
                    <input type="text" name="batch-${index}" placeholder="e.g., BT001">
                </div>
                <div class="dispensing-field">
                    <label>Expiry Date</label>
                    <input type="date" name="expiry-${index}">
                </div>
            </div>
        `;
...
function displayDispensingHistory(history) {
    ...
    if (history.length === 0) {
        historyContainer.innerHTML = `
            <div class="empty-state">
                ...
                <p>No previous medicine dispensing found for this patient</p>
            </div>
        `;
        return;
    }
    ...
    history.forEach(item => {
        ...
        const dispensingDate = new Date(item.visitDate).toLocaleDateString();
        const dispensingTime = new Date(item.visitDate).toLocaleTimeString();
        
        historyItem.innerHTML = `
            <div class="history-header">
                <div class="history-date">${dispensingDate} ${dispensingTime}</div>
...
async function updatePharmacyStatistics() {
    try {
        ...
        allVisits.forEach(doc => {
            ...
            // Count today's completed dispensing
            if (visit.journeyStatus?.pharmacy?.status === 'completed' && 
                visit.journeyStatus?.pharmacy?.timestamp) {
                const dispensingDate = visit.journeyStatus.pharmacy.timestamp.toDate();
                if (dispensingDate >= todayStart && dispensingDate < todayEnd) {
                    todayDispensedCount++;
                }
            }
            
            // Count pending dispensing

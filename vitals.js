...
async function loadCurrentCamp() {
    ...
    try {
        // Update camp display in sidebar or header
        updateCampDisplay();
    } catch (error) {
...
function updateCampDisplay() {
    ...
    // Update the camp info display (you can modify this based on your UI)
    const campDisplay = document.querySelector('.camp-info-display');
    if (campDisplay) {
        campDisplay.innerHTML = `
            <div class="current-camp-info">
                <h4>${currentCamp.name}</h4>
                <p>${currentCamp.location} • ${formatDate(currentCamp.date)}</p>
            </div>
        `;
    }
}
...
// Add event listeners for camp selection
document.addEventListener('DOMContentLoaded', function() {
